...
export const BLOG_API_BASE = "https://jayj-fe.github.io/blogAPI";
...
export const getPosts = cache(async function getPosts() {
  ...
  const localPosts = readLocalJsonPosts();
  const source = localPosts.length > 0 ? localPosts : fallbackPosts.map(normalizePost);
